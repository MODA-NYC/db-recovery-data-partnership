...
DECLARE
    ...
    _view_grouped_weekly_zipcode boolean;
    _view_grouped_daily_zipcode boolean;
BEGIN
    ...
    SELECT 'foursquare_datacube.grouped_daily_zipcode' IN (SELECT table_schema||'.'||table_name FROM information_schema.tables) INTO _view_grouped_daily_zipcode;
    SELECT 'foursquare_datacube.grouped_weekly_zipcode' IN (SELECT table_schema||'.'||table_name FROM information_schema.tables) INTO _view_grouped_weekly_zipcode;
    ...
    IF NOT _view_grouped_daily_zipcode THEN
        CREATE VIEW foursquare_datacube.grouped_daily_zipcode AS (
            SELECT
                date,
                zip as zipcode,
                categoryname as category,
    ...
    IF NOT _view_grouped_weekly_zipcode THEN
        CREATE VIEW foursquare_datacube.grouped_weekly_zipcode AS (
            SELECT
                to_char(date::date, 'IYYY-IW') as year_week,
                zip as zipcode,
                categoryname as category,
